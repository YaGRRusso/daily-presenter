...
export class ConvocationsService {
  constructor(
    ...
    private readonly ConvocationModel: Model<Convocation>,
    private readonly usersService: UsersService
  ) {}

  private readonly shuffleAndSlice = (array: any[], slice?: number) => {
    return array
      .sort(() => 0.5 - Math.random())
      .slice(0, slice ?? array.length);
  };
  ...
  async create(
    ...
    const parsedExpiresAt =
      expiresAt instanceof Date ? expiresAt : AdjustDateEasy(expiresAt);
  ...
  async findAll(findConvocationDto?: FindConvocationDto, method?: QueryMethod) {
    return await this.ConvocationModel.find(
      ApplyQuery(findConvocationDto, method)
    )
      .populate([
        { path: "selectedUsers", select: ["username", "slackId"] },
        { path: "invitedUsers", select: ["username"] },
  ...
  async findOne(findConvocationDto?: FindConvocationDto) {
    return await this.ConvocationModel.findOne(findConvocationDto)
      .populate([
        { path: "selectedUsers", select: ["username", "slackId"] },
        { path: "invitedUsers", select: ["username"] },
        { path: "createdBy", select: ["username"] },
  ...
  async update(id: string, updateConvocationDto: UpdateConvocationDto) {
    return await this.ConvocationModel.updateOne(
      { _id: id },
  ...
  async remove(id: string) {
    return await this.ConvocationModel.deleteOne({ _id: id }).exec();
  }

  // https://app.slack.com/block-kit-builder
  async slack(user?: JwtUser) {
    const baseUrl =
      "http://ec2-18-231-212-234.sa-east-1.compute.amazonaws.com:3000";

    const lastWeekDeployerUsers = await this.ConvocationModel.find({
      key: { $regex: /^week-deployer/ },
    }).then((convocations) =>
      convocations.flatMap((convocation) =>
        convocation.selectedUsers.map(String)
      )
    );
    ...
    const everyoneIds = users.map((usr) => usr.id);
    const developersIds = users
      .filter((usr) => usr.job === "developer")
      .map((usr) => usr.id);
    const availableDevelopersIds = developersIds.filter(
      (id) => !lastWeekDeployerUsers.includes(id)
    );
    ...
    const deployer = await this.findOneOrCreate(
      {
        ...
        key: deployerKey,
        selectedLength: 1,
        invitedUsers: availableDevelopersIds.length
          ? availableDevelopersIds
          : developersIds,
    ...
    const generatePresentersList = (usersList: User[]) => {
      return usersList.map((usr, index) => {
        const { day, date } = currentWeekDays[index + 1];
        return `- ${day} (${date.toLocaleDateString("pt", { day: "2-digit", month: "2-digit" })}): ${usr.slackId ? `<@${usr.slackId}>` : usr.username}`;
      });
    };

    const generateDeployerList = (usersList: User[]) => {
      return usersList.map((usr) => {
        return `${usr.slackId ? `<@${usr.slackId}>` : usr.username}`;
      });
    };
